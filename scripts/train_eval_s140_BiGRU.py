...
# Custom Dataset class for Sentiment140
class TokenizedDataset(Dataset):
    def __init__(self, csv_file):
        data = pd.read_csv(csv_file)
        self.input_ids = data["input_ids"].apply(eval).tolist()  # Convert string to list
        self.labels = data["sentiment"].replace({4: 1}).tolist()  # Convert 4 (positive) to 1
...
def main():
    ...
    # Load data
    train_loader = DataLoader(TokenizedDataset("data/sentiment140/train_biGRU_tokenized.csv"), batch_size=BATCH_SIZE, shuffle=True)
    test_loader = DataLoader(TokenizedDataset("data/sentiment140/test_biGRU_tokenized.csv"), batch_size=BATCH_SIZE)
    ...
    results_file = "results/results_sentiment140_BiGRU.csv"
    ...
    torch.save(model.state_dict(), "models/bigru_sentiment140_model.pth")
