...
class TokenizedDataset(Dataset):
    def __init__(self, csv_file):
        ...
        self.attention_mask = data["attention_mask"].apply(eval).tolist()
        
        # Convert sentiment labels (0 = negative, 4 = positive) → (0 = negative, 1 = positive)
        self.labels = data["sentiment"].replace({4: 1}).tolist()
...
def main():
    ...
    # Load dataset
    train_loader = DataLoader(TokenizedDataset("data/sentiment140/train_distilBERT_tokenized.csv"), batch_size=BATCH_SIZE, shuffle=True)
    test_loader = DataLoader(TokenizedDataset("data/sentiment140/test_distilBERT_tokenized.csv"), batch_size=BATCH_SIZE)
    ...
    results_file = "results/results_sentiment140_DistilBERT.csv"
    ...
    # Save model
    torch.save(model.state_dict(), "models/distilbert_sentiment140_model.pth")
